...
@SuppressLint("NewApi")
public class SaveDT extends AsyncTask<String, Void, String> {
    ...
    public SaveDT(MainActivity activity){
    ...
    protected void onPreExecute() {
        ...
        if (getDialog() != null) {
            getDialog().setMessage("Guardando base de datos...");
            getDialog().show();
        }
    }
    ...
    protected String doInBackground(String... strings) {
        ...
        String v3 = getActivity().getExternalFilesDir("").getPath() + "/AUStock/Databases/";
        MainActivity.exportDB(getActivity());
        File databas = new File(v3 + DATABASE_NAME);
        String database = encodeFileToBase64(databas);

        try {
            URL url= new URL(Preferencias.with(getActivity()).read("DATABASE", ""));
            ...
            String insert_data = URLEncoder.encode("email", "UTF-8")+"="+URLEncoder.encode(Preferencias.with(getActivity()).read("USER", ""), "UTF-8")+
                    "&"+URLEncoder.encode("pass", "UTF-8")+"="+URLEncoder.encode(Preferencias.with(getActivity()).read("PASS", ""), "UTF-8")+
                    "&"+URLEncoder.encode("database", "UTF-8")+"="+URLEncoder.encode(database, "UTF-8");
    ...
    protected void onPostExecute(String s) {
        ...
        if(s.equals("Sin base de datos")){
            Toast.makeText(activity,"No hemos podido encontrar una base de datos", Toast.LENGTH_LONG).show();
            return;
        }
        if(s.contains("Guardado correctamente")){
            Toast.makeText(activity,s,Toast.LENGTH_SHORT).show();
            Preferencias.with(getActivity()).write("USER", "");
            Preferencias.with(getActivity()).write("PASS", "");
            Preferencias.with(getActivity()).write("MYDB", "");
            Toast.makeText(getActivity(),s,Toast.LENGTH_SHORT).show();
            Intent mrz = new Intent(getActivity(), LoginActivity.class);
            getActivity().startActivity(mrz);
            getActivity().finish();
            return;
        }
        Toast.makeText(activity,s,Toast.LENGTH_SHORT).show();
    ...
    private String getUniqueId(Context ctx) {
        @SuppressLint({"HardwareIds", "NewApi", "InlinedApi", "LocalSuppress"}) String key = (getDeviceName() + Settings.Secure.getString(ctx.getContentResolver(), Settings.Secure.ANDROID_ID) + Build.HARDWARE).replace(" ", "");
