...
import com.mrz.austock.activity.RegisterActivity;
...
public class Register extends AsyncTask<String, Void, String> {
    ...
    public Register(RegisterActivity activity){
    ...
    protected void onPreExecute() {
        ...
        if (getDialog() != null) {
            getDialog().setMessage("Iniciando Registro...");
            getDialog().show();
        }
    }
    ...
    protected String doInBackground(String... strings) {
        ...
        try {
            ...
            data.put("pass", strings[2]);
            data.put("cs", "");

            token.put("Data", encrypt(data.toString(), puk));
            token.put("Hash", Stufs.SHA256(data.toString()));

            HttpURLConnection urlConnection = (HttpURLConnection) new URL(strings[3]).openConnection();
            ...
            return Stufs.fromBase64String(Stufs.readStream(urlConnection.getInputStream()));
        } catch (Exception e){
    ...
    protected void onPostExecute(String s) {
        ...
        try {
            ...
            String decData = Stufs.profileDecrypt(ack.get("Data").toString(), ack.get("Hash").toString());
            if (!verify(decData, ack.get("Sign").toString(), puk)) {
                Toast.makeText(activity, "La informacion es incorrecta!", Toast.LENGTH_LONG).show();
                return;
            }
            ...
            if(data.get("Status").toString().equals("Success")) {
                ...
                if(!ver.equals(version)) {
                    ...
                    Toast.makeText((activity),"Te has registrado correctamente",Toast.LENGTH_SHORT).show();
                    Preferencias prefs;
                    prefs = Preferencias.with(getActivity());
                    prefs.write("AUTH", "TRUE");
                    prefs.write("USER", prefs.read("CORREO", ""));
                    prefs.write("PASS", prefs.read("CONTRASEÑA", ""));
