                ...
                public void onClick(View v) {
                    ...
                    builder.setPositiveButton(R.string.yes_expense_delete, new DialogInterface.OnClickListener() {
                        public void onClick(DialogInterface dialog, int which) {
                            ...
                            Cursor cursor = sqLiteDatabase.rawQuery("select * from " + MONEY_TABLE_NAME + " where " + ProductContract.ProductEntry._ID +" = " + 1, null);
                            if(cursor != null && cursor.moveToFirst()){
                                double Recaudado = cursor.getDouble(cursor.getColumnIndex(ProductContract.ProductEntry.COLUMN_PRODUCT_MONEY));
                                double total = 0;
                                total = Recaudado - Double.parseDouble(expenseAmountView.getText().toString());
                                sqLiteDatabase.execSQL("UPDATE `" + ProductContract.ProductEntry.MONEY_TABLE_NAME + "` SET `" + ProductContract.ProductEntry.COLUMN_PRODUCT_MONEY + "` = '" + total + "' WHERE `" + ProductContract.ProductEntry._ID + "` = " + 1);
                            }
    ...
    public void createOrUpdateExpense() {
        if (validateForm()) {
            ...
            if(cursor != null && cursor.moveToFirst()){
                double Recaudado = cursor.getDouble(cursor.getColumnIndex(ProductContract.ProductEntry.COLUMN_PRODUCT_MONEY));
                double total = 0;
                if(expenseAmountView.getText().toString().equals("-")){
                    total = Recaudado - Double.parseDouble(expenseAmountView.getText().toString());
                } else {
                    total = Recaudado + Double.parseDouble(expenseAmountView.getText().toString());
                }
